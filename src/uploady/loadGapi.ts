...
const loadGapi = ({ gApiScriptIdPrefix = 'uploady-drive-', apiUrl = GOOGLE_API, gsiUrl = GOOGLE_GSI, clientId, getToken, scope }: { clientId: string; scope: string; getToken: boolean; gApiScriptIdPrefix?: string; apiUrl?: string; gsiUrl?: string }) => {
  const initAuth = () => {
    const responseCallbacks: Function[] = [];

    const tokenClient = getToken ? undefined : gapi.client.init({
      client_id: clientId,
      scope,
